...
const UpdateTowerBanner = () => {
  ...
  const fetchSliderDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/GetTowerBannerId/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/updateTowerBanner`, formData);

      const data = response.data;

      if (data.status === 'ok') {
        alert('Slider updated successfully!');
        navigate('/SuperAdmin/AllTowerBanner');
      } else {
...
export default UpdateTowerBanner;
