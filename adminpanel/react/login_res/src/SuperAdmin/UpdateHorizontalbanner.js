...
const UpdateHorizontalTowerBanner = () => {
  const { id } = useParams();
  ...
  const fetchSliderDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/GetHorizontalTowerbannerId/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/updateHorizontalTowerbanner`, formData);

      const data = response.data;

      if (data.status === 'ok') {
        alert('Slider updated successfully!');
        navigate('/SuperAdmin/AllHorizontalBanner');
      } else {
  ...
  return (
    ...
    <div className="add-category-content">
        <h1 className="page-title">Update Tower Banner</h1>
...
export default UpdateHorizontalTowerBanner;
