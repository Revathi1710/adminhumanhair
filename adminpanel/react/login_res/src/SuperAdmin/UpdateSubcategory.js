...
const UpdateSubCategory = () => {
  ...
  const fetchProductDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/GetSubcategoryAdmin/${id}`);
      const data = response.data;
      ...
      if (data.status === 'ok') {
        ...
        setCategory(product.Category); // Ensure product.category matches category _id
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/updateSubcategory`, formData, {
        headers: {
  ...
  return (
    ...
    <div className="add-category-container">
      ...
      <div className="add-category-content">
        <h1 className="page-title">Update Subcategory</h1>
  ...
  value={category} // Ensure this is correctly set
>
