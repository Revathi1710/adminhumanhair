...
const UpdateCategoryVendor = () => {
  ...
  const fetchProductDetails = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/GetCategory/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`${process.env.REACT_APP_API_URL}/updateCategory`, formData, {
        headers: {
